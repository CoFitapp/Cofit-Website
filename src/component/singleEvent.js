...
export default function SingleEvent(props) { 
    ...
    const address = eventData?.event?.address || ''; 
    const wordLimit = 100;
    ...
    const image = eventData?.event?.image || '../images/CoFit-Image-Not-Found-1.jpg';
    const title = eventData?.event?.title || '';
    const startDateString = eventData?.event?.date?.start_date || ''; 
    ...
    return (
        <React.Fragment>
             <Helmet>
                <meta charSet="utf-8" />
                <title>{title}</title>
                <link rel="canonical" href="https://cofitapp.com/" />
            </Helmet>
            {/* Back Button */}
            <Box className='back-events'>
